...
function getLocaleFromFilePath(path: string) {
	const match = /(\.[a-zA-Z-]{2,5})+\.(md|mdx|json)$/.exec(path);
	return match?.[1]?.replace(".", "") ?? config.i18n.defaultLocale;
}
...
// biome-ignore lint/style/noDefaultExport: Library requires default exports for config files
